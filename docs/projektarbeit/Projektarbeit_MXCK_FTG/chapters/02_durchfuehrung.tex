...
\bildplatzhalter{Blockdiagramm: /scan $\to$ scan\_preprocessor\_node $\to$
/autonomous/ftg/scan\_filtered $\to$ follow\_the\_gap\_v0 (input\_mode=scan) $\to$
/final\_heading\_angle + /gap\_found $\to$ ftg\_planner\_node $\to$
/autonomous/ftg/gap\_angle + /autonomous/ftg/target\_speed $\to$
ftg\_command\_node $\to$ /autonomous/ackermann\_cmd}{Verarbeitungspipeline
des FTG-Stacks (Blockdiagramm)}
\label{fig:pipeline}
...
Beim Start werden vier ROS~2-Prozesse initiiert:
\texttt{scan\_preprocessor\_node}, \texttt{follow\_the\_gap},
\texttt{ftg\_planner\_node} und \texttt{ftg\_command\_node}.
TF (\texttt{robot\_state\_publisher}) wird separat gestartet.
